...
class BeamDecoder(object):
    ...
    def decode(self, input_ids):
        ...
        self.probabilities = self.softmax(self.result[0][:, self.masked_indexes].topk(10).values)
        ...
        for j, masked_index in enumerate(self.masked_indexes):
            ...
            for i in range(len(self.parent_nodes)):
                ...
                probabilities = self.softmax(result[0][:, self.masked_indexes].topk(10).values)
            ...
            self.parent_nodes = np.asarray(childre_nodes)[current_probs_order[:self.beam_width]].tolist()
            # print("")
